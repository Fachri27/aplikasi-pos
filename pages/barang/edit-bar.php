                                    ...
                                    <form action="act_update.php" method="post">
                                        <div class="form-row">
                                            <div class="form-group col-md-4">
                                                <label class="card-title">Nama Barang</label><br>
                                                <input type="hidden" name="id" value="<?= $data['id_barang'] ?>">
                                                <input type="hidden" name="nam_barlama" value="<?= $data['nama_barang'] ?>">
                                                <input type="text" name="nam_bar" value="<?= $data['nama_barang'] ?>" class="form-control">
                                            </div>
                                            <div class="form-group col-md-4">
                                                <label class="card-title">Kategori</label>
                                                <select name="kategori" class="form-control">
                                                    <?php if($data['id_kategori']==""){ ?>
                                                    <option value="0">-pilih kategori</option>
                                                    <?php }
                                                    include '../../library/koneksi.php';
                                                    $query1 = mysqli_query($conn, "SELECT * FROM tbl_kategori");
                                                    while ($data1 = mysqli_fetch_array($query1)) {
                                                        if($data['id_kategori']==$data1['id_kategori']){
                                                    ?>
                                                        <option value="<?= $data1['id_kategori'] ?>" selected><?= $data1['nama_kategori'] ?></option>
                                                    <?php }else{ ?>
